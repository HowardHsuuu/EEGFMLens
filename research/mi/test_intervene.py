"""Frozen held-out variant of the validation intervention runner.

The intervention loop is kept identical; only protocol guards and partition labels differ.
"""
...
def verify_protocol(protocol, split, prepared, probes, name):
    lock = json.loads(protocol.read_text())
    if sha(split) != lock["split_sha256"]:
        raise ValueError("Frozen split changed")
    for filename, digest in lock["source_hashes"].items():
        if sha(Path(__file__).with_name(filename)) != digest:
            raise ValueError(f"Frozen source changed: {filename}")
    identity = lock["models"][name]
    if (
        sha(probes / "manifest.json") != identity["probe_manifest_sha256"]
        or sha(probes / "probes.npz") != identity["probe_artifact_sha256"]
    ):
        raise ValueError("Frozen probes changed")
    config = (
        lock["input_recipe"],
        lock["middle_site"],
        lock["final_site"],
    ...
    if config != (
        ...
        raise ValueError("Protocol differs from implemented experiment")
    metadata = json.loads((prepared / "manifest.json").read_text())
    if metadata.get("split_sha256") != lock["split_sha256"]:
        raise ValueError("Prepared split provenance differs")
    data = np.load(prepared / "trials.npz", allow_pickle=False)
    if set(data["subjects"]) != set(json.loads(split.read_text())["test"]):
        raise ValueError("Test subject membership differs")


def run(prepared, probes, name, root, output, protocol, split):
    verify_protocol(protocol, split, prepared, probes, name)
    source = json.loads((prepared / "manifest.json").read_text())
    if source.get("partition") != "test":
        raise ValueError("This frozen runner only accepts the test partition")
    ...
    print(name, len(values), "test trial responses saved")
...
if __name__ == "__main__":
    ...
    a = p.parse_args()
    run(a.prepared, a.probes, a.model, a.root, a.output, a.protocol, a.split)
