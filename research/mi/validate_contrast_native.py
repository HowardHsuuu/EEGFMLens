"""Validate minimum-norm contrast edits against independent native hooks."""
...
from eeglens import Replacement, SignalBatch
...
def run(name, root, output):
    ...
    for rank in (2, 4, 8, 16, 32):
        ...
        donor = contrast_donor(activation, batch, q, torch.tensor(center_difference))
        ...
        def hook(module, args, original):
            if name == "labram":
                left = slice(1 + CHANNELS.index("C3") * 4, 1 + (CHANNELS.index("C3") + 1) * 4)
                right = slice(1 + CHANNELS.index("C4") * 4, 1 + (CHANNELS.index("C4") + 1) * 4)
            else:
                left = (
                    order.index(CHANNELS.index("C3")) if name == "csbrain" else CHANNELS.index("C3")
                )
                right = (
                    order.index(CHANNELS.index("C4")) if name == "csbrain" else CHANNELS.index("C4")
                )
            difference = original[:, right].mean(1) - original[:, left].mean(1)
            residual = difference - ((difference - torch.tensor(center_difference)) @ q) @ q.T
            shift = (difference - residual)[:, None] / 2
        ...
        tensor = actual.cache[site].tensor
        before = activation.tensor
        if activation.layout == "tokens":
            torch.testing.assert_close(tensor[:, :1], before[:, :1], atol=0, rtol=0)
            tensor = tensor[:, 1:].reshape(2, 19, 4, -1)
            before = before[:, 1:].reshape(2, 19, 4, -1)
        torch.testing.assert_close(tensor[:, untouched], before[:, untouched], atol=0, rtol=0)
        left, right = CHANNELS.index("C3"), CHANNELS.index("C4")
        ...
        torch.testing.assert_close(
            tensor - tensor.mean(2, keepdim=True),
            before - before.mean(2, keepdim=True),
            atol=5e-5,
            rtol=5e-5,
        )
        records.append(
            dict(
                ...
                untouched_sensor_values_exact=True,
                common_mode_and_temporal_deviations_preserved=True,
    ...
    result = dict(
        ...
        intervention_sha256=sha(inspect.getfile(contrast_donor)),
        basis_sha256=sha(folder / "bases.npz"),
        feature_sha256=sha(features),
        prepared_sha256=sha(prepared / "trials.npz"),
        scope="Two real training trials from outer fold 0; native output exact, pooled contrast float32 tolerance 5e-5; minimum-norm mean contrast edit, preserves common mode and patch deviations within float32 tolerance. No task inference.",
    )
