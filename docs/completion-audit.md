...
1. **Execute the configured platform jobs.** `.github/workflows/tests.yml` defines Ubuntu/macOS/Windows × Python 3.10/3.12 installed-wheel jobs. The user supplied `https://github.com/HowardHsuuu/EEGFMLens.git`; it is now configured as `origin`, and a successful remote-ref query returned no refs. No source has been pushed and no remote job has run. Existing local tests cannot substitute for these jobs. The next external action is the initial source push, followed by inspection of job results and artifacts and resolution of failures before claiming platform support.
...
3. **Keep published artifacts consistent with their tested snapshot.** a8 itself was built and audited, and remains unpublished. Later audit-document edits are working-tree records; they are not silently treated as bytes in the already-tested sdist. Any replacement release artifact needs its own digest and archive checks. No upload is authorized merely by the existence of a local package.
