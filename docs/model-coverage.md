...
Loading examples with strict state-dict checks are in `validation/validate.py`. Additional upstream code/weights stay external to the package and are not silently downloaded by these adapters. EEGPT requires 256 Hz and matching contiguous 64-sample patches in the tested configuration; BIOT uses 200 Hz; BENDR uses 256 Hz. Adapters validate declared input contracts but do not perform channel mapping, normalization or resampling.
